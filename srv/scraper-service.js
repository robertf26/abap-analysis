...
taskQueue.error = function (error, task) {
    logger.error(
        `Task experienced an error while processing: ${task.packageName} - ${error}`
    );
};
...
async function processPackageObjects(
    ...
    try {
        ...
        const classes =
            classDetails["asx:abap"]["asx:values"]["DATA"]["TREE_CONTENT"][
        ...
        for (const cls of classRecords) {
            if (cls.OBJECT_TYPE.startsWith("CLAS")) {
                const classDetailResponse = await getClassDetails(
                    cls.OBJECT_URI,
                    csrfToken,
                    cookies
                );
                const parsedClassDetails = await parser.parseStringPromise(
                    classDetailResponse.data
                );
                const classInfo = parsedClassDetails["class:abapClass"].$;

                // Fetch the class source
                const classSourceResponse = await getClassSource(
                    cls.OBJECT_URI,
                    csrfToken,
                    cookies
                );
        ...
        const programResponse = await getProgramInfo(
            packageName,
            csrfToken,
            cookies
        );
        const programDetails = await parser.parseStringPromise(
            programResponse.data
        );
        const programs =
            programDetails["asx:abap"]["asx:values"]["DATA"]["TREE_CONTENT"][
            "SEU_ADT_REPOSITORY_OBJ_NODE"
            ];
        ...
        for (const prog of programRecords) {
            if (prog.OBJECT_TYPE.startsWith("PROG")) {
                const programDetailResponse = await getProgramDetails(
                    prog.OBJECT_URI,
                    csrfToken,
                    cookies
                );
                const parsedProgramDetails = await parser.parseStringPromise(
                    programDetailResponse.data
                );
                ...
                const programRecord = {
                    ...
                    sourceObjectStatus:
                        programInfo["abapsource:sourceObjectStatus"] || "",
                    fixPointArithmetic:
                        programInfo["abapsource:fixPointArithmetic"] || "",
                    activeUnicodeCheck:
                        programInfo["abapsource:activeUnicodeCheck"] || "",
        ...
        const otherResponse = await getPackageStructure(
            packageName,
            csrfToken,
            cookies
        );
        ...
        const otherObjects =
            otherDetails["asx:abap"]["asx:values"]["DATA"]["TREE_CONTENT"][
            "SEU_ADT_REPOSITORY_OBJ_NODE"
            ];

        let otherObjectRecords = Array.isArray(otherObjects)
            ? otherObjects
            : [otherObjects];
        for (const otherObj of otherObjectRecords) {
            if (
                !otherObj.OBJECT_TYPE.startsWith("CLAS") &&
                !otherObj.OBJECT_TYPE.startsWith("PROG")
            ) {
                const otherObjectDetailResponse = await getOtherObjectDetails(
                    otherObj.OBJECT_URI,
                    csrfToken,
                    cookies
                );
                const parsedOtherObjectDetails = await parser.parseStringPromise(
                    otherObjectDetailResponse.data
                );
                const otherObjectInfo =
                    parsedOtherObjectDetails["adtcore:mainObject"].$;
        ...
        logger.error(
            `Failed to fetch objects for package ${packageName}: ${error.message}`
        );
    }
}
